...
def select_predictors_and_view_count_label_from_record(record):
    x = {"desc_input_ids": record["desc_input_ids"], "desc_input_mask": record["desc_input_mask"], "segment_ids": record["segment_ids"]}
...
def load_checkpoint_model(checkpoint_path):
    ...
    list_of_checkpoint_files = gl(glob_pattern)
...
if __name__ == "__main__":
    ...
    with distributed_strategy.scope():
        ...
        ).map(select_predictors_and_view_count_label_from_record)
        ...
        while retries < 5 and not successful_download:
            try:
                ...
                desc_input_ids = tf.keras.layers.Input(shape=(max_seq_length,), name="desc_input_ids", dtype="int32")
                desc_input_mask = tf.keras.layers.Input(shape=(max_seq_length,), name="desc_input_mask", dtype="int32")

                embedding_layer = transformer_model.distilbert(desc_input_ids, attention_mask=desc_input_mask)[0]
                ...
                model = tf.keras.Model(inputs=[desc_input_ids, desc_input_mask], outputs=X)
    ...
    if run_sample_predictions:
        ...
        print(
            """Lost Outside""",
            predict("""Lost Outside"""),
        )

        print("""First 30 Kill Streak in War Zone""", predict("""First 30 Kill Streak in War Zone"""))

        print(
            """First 30 Kill Streak in Call of Duty: War Zone""",
            predict("""First 30 Kill Streak in Call of Duty: War Zone"""),
        )
